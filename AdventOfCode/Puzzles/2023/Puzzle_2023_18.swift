...
public class Puzzle_2023_18: PuzzleBaseClass {
    ...
    private func CalculateArea(_ points: [Point2D], _ totalEdgeLength: Int) -> Int {
        var area = 0
        for i in 0..<(points.count - 1){
            area += (points[i].y + points[i + 1].y) * (points[i].x - points[i + 1].x)
        }

        area += (points.last!.y + points[0].y) * (points.last!.x - points[0].x)
    ...
    private func solvePart1(str: String) -> Int {
        ...
        for line in arr {
            ...
            points.append(Point2D(x: currentX, y: currentY))
        }
    ...
    private func solvePart2(str: String) -> Int {
        ...
        var points = [Point2D]()
        for line in arr {
            var components = line.capturedGroups(withRegex: "(.) (.*) \\(\\#(.*)\\)", trimResults: true)
            let direction = components[2].removeLast()
            let distance = Int(components[2], radix: 16)!
            totalEdgeLength += distance
            switch direction {
            case "0":
                currentX += distance

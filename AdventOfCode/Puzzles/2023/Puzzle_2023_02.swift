...
public class Puzzle_2023_02: PuzzleBaseClass {
    public func solve() {
    ...
    private func gameIsPossible(_ arr: [String]) -> Bool {
        for draw in arr {
    ...
    private func possibleValues(_ arr: [String]) -> (Int, Int, Int) {
        var maxBlue = 0
        var maxRed = 0
        var maxGreen = 0
        for draw in arr {
    ...
    private func solvePart2(str: String) -> Int {
        ...
        for line in arr {
            let arr1 = line.description.parseIntoStringArray(separator: ":").map { $0.trim().replacingOccurrences(of: "Game ", with: "") }
            let arr2 = arr1[1].parseIntoStringArray(separator: ";").map { $0.trim() }
            let values = possibleValues(arr2)
            retval += values.0 * values.1 * values.2
        }

...
public class Puzzle_2023_08: PuzzleBaseClass {
    ...
    private func parseInput(_ str: String) -> ([String.Element], [String: (String, String)]) {
        var arr = str.parseIntoStringArray()
        let stepMap = Array(arr.removeFirst())
        var dict = [String: (String, String)]()
        for line in arr {
            let components = line.capturedGroups(withRegex: "(.*) = \\((.*), (.*)\\)", trimResults: true)
            dict[components[0]] = (components[1], components[2])
        }
    ...
    private func solvePart1(str: String) -> Int {
        let (stepMap, dict) = parseInput(str)
        var moves = 0
        var position = "AAA"
        while position != "ZZZ" {
            let nextNode = dict[position]!
            if stepMap[moves % stepMap.count] == "L" {
                position = nextNode.0
            } else {
                position = nextNode.1
            }
    ...
    private func solvePart2(str: String) -> Int {
        let (stepMap, dict) = parseInput(str)
        let originalPositions = dict.keys.filter { $0.hasSuffix("A") }
        let ghostCounter = originalPositions.count
        var moveArray = [Int]()
        for idx in 0..<ghostCounter {
            var moves = 0
            var position = originalPositions[idx]

...
class Puzzle_2015_07 : PuzzleBaseClass {
    ...
    func solveBothParts() -> (Int, Int) {
        ...
        var wireDictionary = Dictionary<String, UInt16>()

        func isAvailable(_ theString: String) -> Bool {
            if theString.isStringNumeric() {
                return true
            }
        ...
        func processPart(partNumber: Int, specialValue: UInt16) {
            var operationArray = Array<Operation>()
            ...
            for part in partsArray {
                ...
                if componentsArray[1] == "->" {
                    op.opcode = "->"
                    op.op1 = componentsArray[0]
                    op.opResult = componentsArray[2]
                } else if componentsArray[0] == "NOT" {
                    op.opcode = "NOT"
                    op.op1 = componentsArray[1]
                    op.opResult = componentsArray[3]
                } else {
                    op.opcode = componentsArray[1]
                    op.op1 = componentsArray[0]
                    op.op2 = componentsArray[2]
                    op.opResult = componentsArray[4]
                }
                    ...
                    if !op.processed {
                        if partNumber == 2 && op.opResult == "b" {
                            ...
                            if isAvailable(op.op1) && isAvailable(op.op2) {
                                if op.opcode == "AND" {
                                    wireDictionary[op.opResult] = wireValue(op.op1) & wireValue(op.op2)

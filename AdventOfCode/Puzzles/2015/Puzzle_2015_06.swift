...
public class Puzzle_2015_06: PuzzleBaseClass {
    ...
    public func solveBothParts() -> (Int, Int) {
        ...
        for command in commandArray {
            ...
            let toggleMode = (tokenArray[0] == "toggle")
            let lightMode = (tokenArray[1] == "on")
            for x in startingCoordinates.0 ... endingCoordinates.0 {
                for y in startingCoordinates.1 ... endingCoordinates.1 {
                    if toggleMode {
                        lightArray[x][y] = !lightArray[x][y]
                        brightnessArray[x][y] = brightnessArray[x][y] + 2
                    } else {
                        lightArray[x][y] = lightMode
                        if lightMode {
                            brightnessArray[x][y] = brightnessArray[x][y] + 1
                        } else {
                            brightnessArray[x][y] = max(brightnessArray[x][y] - 1, 0)
                        }
                    }
                }
            }

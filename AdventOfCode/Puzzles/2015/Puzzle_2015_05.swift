...
public class Puzzle_2015_05: PuzzleBaseClass {
    public func solve() {
    ...
    public func solveBothParts() -> (Int, Int) {
        func hasDuplicateCharacters(_ theWord: String) -> Bool {
            for c in "abcdefghijklmnopqrstuvwxyz" {
                if theWord.contains("\(c)\(c)") {
                    return true
                }
            }
        ...
        func isWordNicePart1(_ theWord: String) -> Bool {
            var atLeastThreeVowels = false
            let matches = theWord.matchesForRegexInText(regex: "[aeiou]")
            if matches.count >= 3 {
                atLeastThreeVowels = true
            }

            let containsDuplicates = hasDuplicateCharacters(theWord)

            var containsInvalidString = false
            if theWord.contains("ab") || theWord.contains("cd") || theWord.contains("pq") || theWord.contains("xy") {
                containsInvalidString = true
            }

            return atLeastThreeVowels && containsDuplicates && !containsInvalidString
        }

        func hasNonOverlappingPairs(_ theWord: String) -> Bool {
        ...
        func hasOneLetterInBetween(_ theWord: String) -> Bool {
            var foundMatch = false
            for idx in 0 ..< theWord.count - 2 {
                let c0 = theWord[idx]
                let c1 = theWord[idx + 2]
                if c0 == c1 {
                    foundMatch = true
                }
            }
        ...
        func isWordNicePart2(_ theWord: String) -> Bool {
            let nonOverlappingPairs = hasNonOverlappingPairs(theWord)
            let oneLetterInBetween = hasOneLetterInBetween(theWord)
